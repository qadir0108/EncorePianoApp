...
public class AcknowledgmentService extends BaseService {
	...
	public boolean AcknowledgeConsignment(String RunsheetId) throws UrlConnectionException, JSONException, JSONNullableException, NotConnectedException, NetworkStatePermissionException, DatabaseInsertException, ClientProtocolException, IOException{
		...
		jsonData.object();
		jsonData.key(DataSyncModelEnum.AuthToken.Value).value(ServiceUtility.loginService.LoginModel.getAuthToken());
	...
	public boolean AcknowledgeMessage(String MessageId) throws UrlConnectionException, JSONException, JSONNullableException, NotConnectedException, NetworkStatePermissionException, DatabaseInsertException, ClientProtocolException, IOException{
		
		this.MessageId = MessageId;
		boolean isAcknowledged = true;
		
		HttpPost postRequest = new HttpPost(ServiceUrls.GetMessageAckUrl(context));
		...
		jsonData.object();
		jsonData.key(DataSyncModelEnum.AuthToken.Value).value(ServiceUtility.loginService.LoginModel.getAuthToken());
		jsonData.key(DataSyncModelEnum.MessageID.Value).value(this.MessageId);
		jsonData.key(DataSyncModelEnum.Acknowledged.Value).value(isAcknowledged);
		jsonData.endObject();

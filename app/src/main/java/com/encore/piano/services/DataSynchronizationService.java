...
public class DataSynchronizationService extends BaseService {
	...
	public DataSynchronizationService(Context context)
		...
		this.VehicleCode = ServiceUtility.loginService.LoginModel.getFCMToken();
	}
	...
	{
		ConsignmentModel cm = Database.GetConsignments(context, false, false, consignmentId).get(0);
		...
		if (SendDataSyncContent(CreateDataSyncModel(cm), 1, 1))
			MarkConsignmentAsSynced(cm.getId());

		if (Database.AreAllConsignmentsSynced(context))
			FinalizeConsignmentSynchronization(cm.getId(), 3);
	}

	public int GetNonSyncConsignmentCount()
	{
		return Database.GetConsignments(context, false, false, "").size();
	}
	...
	public void SynchronizeConsignments() throws ClientProtocolException, IOException, JSONNullableException, EmptyStringException
	{
		...
		ArrayList<ConsignmentModel> NonSyncedConsignments = new ArrayList<ConsignmentModel>();
		ArrayList<DataSyncModel> DataSyncItems = new ArrayList<DataSyncModel>();

		NonSyncedConsignments = Database.GetConsignments(context, false, false, "");
		...
		else
		{
			...
			int j = 0;
			for (ConsignmentModel cm : NonSyncedConsignments)
			{
			...
			for (int i = 0; i < DataSyncItems.size(); i++)
			{
				try
				{
					...
					{
						MarkConsignmentAsSynced(DataSyncItems.get(i).getConsignmentId());
	...
	private DataSyncModel CreateDataSyncModel(ConsignmentModel cm)
	{
		DataSyncModel model = new DataSyncModel();

//		ArrayList<GalleryModel> ConsignmentImages = Database.GetImagesForConsignment(context, cm.getId());
//
...
//
//		model.setConsignmentId(cm.getId());
//		model.setAuthToken(ServiceUtility.LoginService.LoginModel.getAuthToken());
...
//		model.setArrivalTime(cm.getArrivalTime());
...
//		model.setSaved(cm.isSaved());
	...
	{
		UpdateProgressListener(count, step, "Syncing consignments", "Sending consignmentService data.");
		...
		if (responseEntity != null)
		{
			...
			{
				UpdateProgressListener(count, step, "Syncing consignments", "Sending consignmentService images " + ++i + " of " + model.getImages().length + ".");
	...
	private void MarkConsignmentAsSynced(String consignmentId) throws DatabaseUpdateException
	{
		Database.SetConsignmentSynced(context, consignmentId);
	}

	public boolean AreAllConsignementsSynced()
	{
		return Database.GetConsignments(context, false, false, "").size() ==0;
	}
